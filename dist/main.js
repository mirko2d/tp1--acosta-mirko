...
for (let i = 1; i <= 22; i++) {
    jugadoresFutbol.push(new Jugador(`f${i}`, `Jugador Fútbol ${i}`, 20 + (i % 10), i % 2 === 0 ? 'Defensa' : 'Delantero'));
}
const jugadoresBasquet = [];
for (let i = 1; i <= 10; i++) {
    jugadoresBasquet.push(new Jugador(`b${i}`, `Jugador Básquet ${i}`, 22 + (i % 5), i % 2 === 0 ? 'Alero' : 'Base'));
}
...
try {
    ...
    try {
        ...
        console.log('Error esperado al programar partido con ID duplicado:', error instanceof Error ? error.message : String(error));
    }
    console.log('\n=== JUGANDO PARTIDOS ===');
